...
var watcher = hound.watch('./files');
watcher.on('create', function(file, stats) {
    logger.logInfo('xmlXlsxParserMain - watcher.on files directory: ' + file);
    xmlXlsxParser(file);
});

function xmlXlsxParser(file) {
    var now = new Date(); 
    now.setHours(0);
    now.setMinutes(0);
    now.setSeconds(0);
    ...
    var fullPathName = __dirname + file.slice(1, file.length);
    if(fs.statSync(fullPathName).isFile()) {
        var ext =  fullPathName.split('.').pop().toLowerCase();
        switch(ext) {
        case 'xlsx':
            xlsxFiles.push(fullPathName);
            break;
        case 'xml':
            xmlFiles.push(fullPathName);
            break;
        }
    }
    ...
    async.waterfall([
        ...
        function(err) {
            if (err) {
                ...
                logger.logError(err);
                sendMail('parsing xlsx/xml files failed', err, null);
            } else {
                logger.logInfo('xmlXlsxParserMain - parsing xlsx/xml files succeed!');
                sendMail('parsing xlsx/xml files succeed', 'done', null);
            }
        }
    );
};
