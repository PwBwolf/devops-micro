...
var config = require('../../server/common/setup/config'),
    ...
    logFile = fs.createWriteStream(__dirname + '/compare-premium-fs-db-packages.log', {flags: 'w'});
...
MongoClient.connect(config.db, function (err, db) {
    if (err) {
        ...
        accounts.find({'type': 'paid'}).toArray(function (err, accounts) {
            if (err) {
                logger.logError('adminCLI - comparePremiumFsDbPackages - error fetching accounts');
                logger.logError(err);
                process.exit(1);
            } else if (!accounts || accounts.length === 0) {
                logger.logError('adminCLI - comparePremiumFsDbPackages - no accounts found!');
                process.exit(0);
            } else {
                var csv = '';
                var count = 0;
                        ...
                        users.findOne({_id: account.primaryUser}, function (err, user) {
                            if (err) {
                                ...
                                billing.login(user.email, account.key, user.createdAt.getTime(), function (err, sessionId) {
                                    if (err) {
                                        logger.logError('adminCLI - comparePremiumFsDbPackages - error logging into freeside for user ' + user.email);
                                        logger.logError(err);
                                        callback();
                                    } else {
                                        billing.getPackages(sessionId, function (err, packages) {
                                            if (err) {
                                                logger.logError(err);
                                                logger.logError('adminCLI - comparePremiumFsDbPackages - error getting packages for user ' + user.email);
                                                callback();
                                            } else {
                                                var pkg = [];
