...
Account.find(query).populate('primaryUser').exec(function (err, accounts) {
    if (err) {
    	...
    	console.log('"Email","First Name","Last Name","Telephone","Status","Freeside Customer Number","User Create Date","Account Create Date","User Cancel Date","Cancel On Date","Merchant"');
        for (var i = 0; i < accounts.length; i++) {
            console.log(
                    ...
                    formatDate(accounts[i].primaryUser.createdAt) + ',' + formatDate(accounts[i].createdAt) + ',' + formatDate(accounts[i].primaryUser.cancelDate) + ',' + formatDate(accounts[i].primaryUser.cancelOn) + ','  +
