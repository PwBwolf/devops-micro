...
var logFile = fs.createWriteStream(__dirname + '/migrate-canceled-users.log', {flags: 'w'});
...
Accounts.find({type: 'paid'}).populate('primaryUser').exec(function (err, accounts) {
        ...
        async.eachSeries(
            ...
            function (account, callback) {
                if (account.primaryUser && account.primaryUser.status === 'canceled') {
                    counter++;
...
function migrate(account, cb) {
    ...
    async.waterfall([
        ...
        function (callback) {
            ...
            account.primaryUser.oldInactiveUser = 300;
