...
var logFile = fs.createWriteStream(__dirname + '/migrate-payment-pending-users.log', {flags: 'w'});
...
Accounts.find({type: 'paid'}).populate('primaryUser').exec(function (err, accounts) {
        ...
        async.eachSeries(
            ...
            function (account, callback) {
                if (account.primaryUser && account.paymentPending === true) {
                    counter++;
...
function migrate(account, cb) {
    ...
    async.waterfall([
        ...
        function (callback) {
            ...
            account.primaryUser.oldInactiveUser = 400;
