...
var logFile = fs.createWriteStream(__dirname + '/migrate-trial-ended-users.log', {flags: 'w'});
...
Accounts.find({type: 'free'}).populate('primaryUser').exec(function (err, accounts) {
        ...
        async.eachSeries(
            ...
            function (account, callback) {
                if (account.primaryUser && account.primaryUser.status === 'trial-ended') {
                    counter++;
...
function migrate(account, cb) {
    ...
    async.waterfall([
        ...
        function (callback) {
            ...
            account.primaryUser.oldInactiveUser = 100;
