...
var logFile = fs.createWriteStream(__dirname + '/migrate-comp-active-users.log', {flags: 'w'});
...
function migrate(account, cb) {
    ...
    async.waterfall([
        ...
        function (callback) {
            aio.updateUserPackages(email, config.aioComplimentaryUserPackages, function (err) {
                if (err) {
