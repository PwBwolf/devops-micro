    ...
    async.waterfall([
                ...
                Image.find({type: 'channel'}, function (err, images) {
                    if (err) {
                        ...
                        for (var i = 0; i < images.length; i++) {
                            imageUriUniq.push({
                                uri: images[i].preferredImage.uri,
                                type: images[i].type,
                                status: 'saved',
                                id: images[i].identifier
                            });
                        }
            ...
            function (images, callback) {
                graceNote.getChannelList(function (err, data) {
                    ...
                    for (i = 0; i < imageUriUniqTemp.length; i++) {
                        var isImageInTempUniq = true;
                        for (j = 0; j < imageUriUniqCount; j++) {
                                ...
                                if (err) {
                                    ...
                                    if (dataPrograms[0].airings === undefined) {
                                        ...
                                        for (var i = 0; i < airings.length; i++) {
                                            imageUri.push({
                                                uri: airings[i].program.preferredImage.uri,
                                                type: 'program',
                                                status: 'new',
                                                id: airings[i].program.tmsId
                                            });
                                        }
                                ...
                                if (removed.length === 0) {
                                    ...
                                    ImageData.remove({_id: removed[0].dataId}, function (err, entries) {
                                        if (err) {
                                            ...
                                            Image.remove({
                                                type: 'program',
                                                'preferredImage.uri': imageRemoved.uri
