...
describe('Controller: forgotPasswordCtrl', function () {
        ...
        function mockTranslateFilter(value) {
            switch (value) {
                case 'FORGOT_PASSWORD_ERROR':
                    return 'Error sending reset password link. Please contact customer support at';
                    break;
                default:
                    return '';
            }
    ...
    describe('forgot Password', function () {
        ...
        it('should log appropriate error message on internal server error', function () {
            ...
            httpBackend.flush();
            expect(location.path()).toBe('/');
            expect(loggerService.logError).toHaveBeenCalledWith('Error sending reset password link. Please contact customer support at 800-123-1111');
            expect(scope.saving).toBe(false);
        });
